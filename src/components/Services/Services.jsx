        ...
        <div>
            <div className="w-[90%] overflow-hidden mx-auto md:w-full overflow-y-hidden h-72 group hover:bg-gradient-to-b transition-colors duration-100 hover:from-[#DEE4EA] hover:to-[#F9FCFF] bg-containerColor border-[3px] border-b-firstColor rounded-[6px]  pl-[25px] pr-[60px] service-card">
                <div className="h-full translate-y-10 py-[50px] group-hover:translate-y-0 group-hover:py-[25px] transition-transform duration-500 ">
                    ...
                    <h3 className="text-[1.5em] font-bold leading-7 mb-[20px] text-textColor">{t(service.title)}</h3>
                    <div ref={learmoreBtnRef}
                         className="flex align-center mt-24 gap-2 learn-more-btn text-firstColor cursor-pointer">
                        {t("Learnmore")} <span className="i mt-1"> <FaLongArrowAltRight/> </span>
                    </div>
                </div>

            </div>
            <div ref={serviceModalsRef}
                 className="service-modal z-30 fixed w-full  md:h-[100vh] top-0 left-0 bg-transparent backdrop-blur-[10px] flex items-center justify-center ">
                <div
                    className="service-modal-body relative overflow-y-auto max-h-[100vh] bg-containerColor max-w-[600px] m-[20px] p-[40px] rounded-[10px] ">
                    <span ref={closeBtnRef} className="absolute top-0 right-0 m-[20px] cursor-pointer "> <MdOutlineClose/> </span>
                    <h3 className="text-[2em]"> {t(service.title)}</h3>
                    <h4 className="text-[1.3em] font-semibold mt-[15px] mx-0 mb-[10px]"> {t("WhatIs") + " "+ t(service.title)} ?</h4>
                    <p>{t(service.desc)}</p>
                    <h4 className="mt-6">{t("WhatIProvide")}</h4>
                    <ul>
                        {/* eslint-disable-next-line react/prop-types */}
                        {service.points.map((point, i) => (
                            <li className="flex mt-[15px] gap-1 items-center" key={i}>
                                <FaCheckCircle className="text-firstColor"/> {t(point)}
                            </li>
                        ))}
                    </ul>
                </div>
